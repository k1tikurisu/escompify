...
export async function getChangedTestFilePaths(repoPath: string, srcHash: string, dstHash: string) {
  ...
  const paths = (await git.diff(['--name-only', `${srcHash}...${dstHash}`]))
    ...
    .filter((path) => {
      ...
      return /(test|spec)/.test(path) && /\.(ts|js)$/.test(path) && !path.endsWith('.d.ts');
    })
